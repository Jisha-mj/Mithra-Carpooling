    ...
    <div v-if="availableCarpools.length" style="background-color:  #fad3a5">
      ...
      <div v-for="carpool in availableCarpools" :key="carpool.id"  class="carpool-box">
        <div class="carpool-info">
          <p :style="{ 'background-color': carpool.status === 'pending' ? 'yellow' : (carpool.status === 'accept' ? 'green' : (carpool.status === 'reject' ? 'red' : 'inherit'))}">
            <strong>Status: </strong>{{ carpool.status }}
          </p>
